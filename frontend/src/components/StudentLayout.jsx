      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-indigo-950 text-white flex flex-col shadow-2xl z-50 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out shrink-0`}>
        <div className="p-6 border-b border-indigo-900/50 bg-indigo-950/50 backdrop-blur-md flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs font-bold text-indigo-300 uppercase tracking-widest">Student</span>
            </div>
          </div>
        ...
        <nav className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
          ...
          >
            {({ isActive }) => (
              <>
                <User size={20} className={`mr-3 transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110 group-hover:text-blue-400'}`} />
                <span className="font-medium">Profile & Pass</span>
              </>
            )}
          </NavLink>
          ...
          >
            {({ isActive }) => (
              <>
                <ShieldCheck size={20} className={`mr-3 transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110 group-hover:text-blue-400'}`} />
        ...
        <div className="p-4 m-4 bg-indigo-900/30 rounded-2xl border border-indigo-800/50 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs text-indigo-300 truncate">{user?.studentId}</p>
            </div>
            <button 
      ...
      <main className="flex-1 h-screen flex flex-col overflow-hidden w-full relative">
        ...
        <header className="md:hidden bg-indigo-950 border-b border-indigo-900/50 px-4 py-3 flex items-center justify-between z-10 relative shadow-sm shrink-0 text-white">
          <div className="flex items-center gap-2.5">
            <img src="/IMG_4244.png" alt="Kingswood Education Center Logo" className="h-8 w-8 rounded-lg object-contain bg-white p-0.5 shadow-md border border-indigo-400/20 shrink-0" />
            <span className="font-black text-white text-lg tracking-tight">Kingswood Student</span>
          </div>
