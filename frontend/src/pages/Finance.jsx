...
const Finance = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/finance/reports?classId=${reportClass}&month=${reportMonth}`);
      setUnpaidStudents(res.data.unpaidStudents || []);
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between sm:items-center bg-slate-50/50">
            <h3 className="text-xl font-bold text-slate-800 flex items-center">
              <FileText className="mr-3 text-indigo-500" size={24} /> Payment Reports
            </h3>
          ...
          <div className="p-0">
            {!reportClass ? (
              <div className="p-12 text-center text-slate-400">
                ...
                <p className="font-medium text-lg">Select a class to view payment reports</p>
              </div>
            ) : reportLoading ? (
              ...
              <div>
                <div className="flex border-b border-slate-100 bg-slate-50/30">
                  <button 
                    onClick={() => setReportFilter('unpaid')}
                    className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${reportFilter === 'unpaid' ? 'border-rose-500 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                  >
                    Unpaid Students ({unpaidStudents.length})
                  </button>
                  <button 
                    onClick={() => setReportFilter('paid')}
                    className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors ${reportFilter === 'paid' ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                  >
                ...
                ) : reportFilter === 'paid' && paidStudents.length === 0 ? (
                  <div className="p-12 text-center text-slate-400">
                    <FileText size={48} className="mx-auto mb-3 opacity-20" />
                    <p className="font-bold text-xl">No Payments Yet</p>
                    <p className="text-sm font-medium mt-1">No one has paid for {format(new Date(reportMonth), 'MMMM')}.</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {(reportFilter === 'unpaid' ? unpaidStudents : paidStudents).map(student => (
                          <tr key={student.studentId} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-4 px-6 text-right">
                              {reportFilter === 'unpaid' ? (
                                ...
                                <span className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 font-bold rounded-lg text-xs border border-emerald-200">
